const CACHE_NAME = 'mibamittaa-v7';
...
self.addEventListener('fetch', event => {
    ...
    if (url.hostname.includes('workers.dev')) {
        event.respondWith(caches.open(API_CACHE).then(cache => fetch(event.request).then(res => { if (res.ok) cache.put(event.request, res.clone()); return res; }).catch(() => cache.match(event.request))));
        return;
    }
